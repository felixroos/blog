// taken from https://github.com/justin-coberly/dagre-d3-react/blob/master/src/index.tsx
// adjusted to allow passing styles to nodes
...
class DagreGraph extends Component<GraphProps> {
  ...
  _drawChart = () => {
    ...
    nodes.forEach((node) => {
      ...
      if (node.style) {
        g.node(node.id).style = node.style;
      }
    });

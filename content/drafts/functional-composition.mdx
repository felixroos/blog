...
const pattern = curry((indices, array) =>
  indices.map((i) => array[i % array.length])
);
...
const nest = (patterns) =>
  pipe(
    ...patterns
      .slice(1)
      .map((pattern) => pipe(nestedDegrees(pattern), flatten)),
    degrees
  )(patterns[0]);
...
<Player
  instruments={{ piano }}
  events={renderRhythm(clark('D4', 'harmonic minor'))}
/>
...
Here we can assign the scale on the parent and inherit it down to the pattern -->

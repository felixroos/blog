...
export const x = console.log(marblemachine);

<Player
  fold={true}
  instruments={{ MK2md2 }}
  events={renderRhythm(marblemachine, [inherit('transpose')])
    .reduce(tieReducer(), [])
    .map(({ value, transpose, ...event }) => ({
      ...event,
      value: Note.midi(value)
        ? Note.transpose(value, Interval.fromSemitones(transpose))
        : value
    }))}
/>
...
<!--<Player
  fold={true}
  instruments={{ MK2md2 }}
  events={rhythmEvents(swimming, [inherit('velocity')])}
/>-->

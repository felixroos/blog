...
import rotateScale from '../components/sets/rotateScale.ts';
    ...
    return (
      <>
        <Grid spacing={0} container style={{ width: '100%' }} alignItems="center">
          <Grid item xs={6}>
            <HarmonicCircle sets={[{ stroke: 'steelblue', set }]} pitches={scale.notes} tonic={scale.tonic} />
            <p>
              Circle of Fifths
              <Button
                color="primary"
                onClick={() => setState({ scale: rotateScale(-4, state.scale, scales) || state.scale })}
              >
                rotate scale
              </Button>
              <Button color="primary" onClick={() => setState({ scale: rotateTonic(4, state.scale, scales) || state.scale })}>
                rotate tonic
              </Button>
              <Button color="primary" onClick={() => setState({ scale: transposeScale(5, state.scale) || state.scale })}>
                transpose
              </Button>
            </p>
          </Grid>
          <Grid item xs={6}>
            <ChromaticCircle sets={[{ stroke: 'steelblue', set }]} pitches={scale.notes} tonic={scale.tonic} />
            <p>
              Chromatic Circle
              <Button
                color="primary"
                onClick={() => setState({ scale: rotateScale(1, state.scale, scales) || state.scale })}
              >
                rotate scale
              </Button>
              <Button color="primary" onClick={() => setState({ scale: transposeScale(-1, state.scale) || state.scale })}>
                transpose
              </Button>
              <Button color="primary" onClick={() => setState({ scale: rotateTonic(1, state.scale, scales) || state.scale })}>
                rotate tonic
              </Button>
            </p>
          </Grid>
        </Grid>
        <ScalePicker scale={state.scale} onChange={(scale) => setState({ scale })} scales={scales} /> Chroma: {
          pcset.chroma
        } = {pcset.setNum} {/*pcset.normalized*/}
      </>
    );
  }}
</State>

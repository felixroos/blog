...
If we divide each index by the number of siblings, we can calculate relative fractions:
        ...
        onNext={(_state) => {
          ...
          const calculation = (
            <React.Fragment>
              ...
              <BlockMath>{`duration = ${duration} = \\frac{${d.n}}{${d.d}}`}</BlockMath>
              <BlockMath>{`time = ${time} = \\frac{${t.n}}{${t.d}}`}</BlockMath>
              <p>By multiplying with 6/4, we get the conventional fractions:</p>
              <BlockMath>{`duration * \\frac{6}{4} = \\frac{${d.n}}{${d.d}} * \\frac{6}{4} = \\frac{${cd.n}}{${cd.d}}`}</BlockMath>
              <BlockMath>{`time * \\frac{6}{4} = \\frac{${t.n}}{${t.d}} * \\frac{6}{4} = \\frac{${ct.n}}{${ct.d}}`}</BlockMath>
            </React.Fragment>
          );
...
<Player
  instruments={{ piano }}
  events={renderRhythm(
    generateVoicings({ duration: 6, sequential: ['C^7', 'Am7', 'Dm7', ['G7', 'G7b9']] }, lefthand, ['G2', 'C5'])
  )}
/>

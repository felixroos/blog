...
| color                      | value | type          | amount    | visual cue                  | sound     |
| -------------------------- | ----- | ------------- | --------- | --------------------------- | --------- |
| <Color color={colors[0]}/> | 3/4   | bar           | 2 total   | seperated by vertical lines | high tom  |
| <Color color={colors[1]}/> | 1/4   | 4ths          | 3 per bar | separated by space          | bass drum |
| <Color color={colors[2]}/> | 1/8   | 8ths          | 2 per 4th | connected by first beam     | hi hat    |
| <Color color={colors[3]}/> | 1/24  | 16th triplets | 3 per 8th | connected by second beam    | snare     |
...
| color                      | value | sound      |
| -------------------------- | ----- | ---------- |
| <Color color={colors[0]}/> | 3/4   | hi hat     |
| <Color color={colors[1]}/> | 1/4   | high tom   |
| <Color color={colors[2]}/> | 1/8   | middle tom |
| <Color color={colors[3]}/> | 1/24  | low tom    |
...
<Player
  ...
  fold={true}
  center={0}
  events={renderRhythm(colorize({ duration: 6, sequential: [
  "sn",
  ["sn", "sn", "sn"],
  "sn",
  ["sn", "sn", "sn"],
...
To know calculate the time fraction, our numerator must be the sum of all durations that came before the node in question:
...
To add variable durations, we can just add a third number to our path. For aesthetic reasons, I chose to add this number in the middle:
...
```js
const renderEvents = (tree, duration, getDuration = (_) => 1) =>
...
And that's it! Let's run the beast on a rhythmical object:
...
- Here, the duration of the root node is interpreted as seconds
- Each

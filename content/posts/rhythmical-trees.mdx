...
export const relativeLabels = ['bolero', '1/2', '1/3', '1/2', '1/3'];
...
What if we invented a 3/3 meter? As everything is relative, we could just multiply each fraction by 4/3, and get this:
...
<Tree
  ...
  data={rhythmicalTree(bolero, (rhythm, path) => ({
    ...toRhythmObject(rhythm),
    color: path.length ? colors[path.length - 1] : 'black',
    name: absoluteLabels[path.length],
  }))}
  hideJson={true}
/>

![bolero](./img/bolero33.png)

This changes just the reference point, while the information stays intact, just more simple.
...
<Tree
  ...
  data={rhythmicalTree(bolero, (rhythm, path) => ({
    ...toRhythmObject(rhythm),
    color: path.length ? colors[path.length - 1] : 'black',
    name: relativeLabels[path.length],
  }))}
  hideJson={true}
/>

Now we get an interesting effect: the denominators always "coincide" with the amount of elements in a group!
To get back to the absolute fraction, we just have to multiply all fractions top down, for example to get the absolute fraction of a 16ths triplet (last row):

<BlockMath>{`3/3 * 1/3 * 1/2 * 1/3 = 1/18`}</BlockMath>

... and to get the "conventional" fraction:

<BlockMath>{`1/18 * 3/4 = 1/24`}</BlockMath>

To convert that fraction to the absolute duration, we just need to know the absolute duration of any node.

### Index Path

We can identify each node in the tree, by a unique index path:

<State initial={{ path: null }}>
  {({ state, setState }) => (
    <>
      <Tree
        width={620}
...
if we now divide the index by its relative subdivision, we get this:
    ...
    <>
      <Tree
        ...
        onClick={(node) => {
          ...
          console.log(d3Path(node));
          setState({ path, fraction, node });
        }} // || pathString(state.node.path).includes(pathString(path))
        ...
        hideJson={true}
      />
      {!state.path && <p>Click a node to show the calculation of its absolute fraction!</p>}
      {state.path && (
        <>
          <p>We can calculate the absolute fraction with the sum of the each node's parent product:</p>
...
```js
const conventionalLabels = ['bolero', '3/4', '1/4', '1/8', '/24'];

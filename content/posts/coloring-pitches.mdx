...
import { ColorKeyboard } from "../components/coloring-pitches/ColorKeyboard"
import { noteColor, noteLightness, rainbow, sinebow } from "../components/coloring-pitches/noteColor"

<ColorKeyboard options={{ scaleX: 0.5, scaleY: 0.5, range: ["C0", "C8"] }} colorizer={rainbow} />
...
<ColorKeyboard
    ...
    colorizer={note => sinebow(note).toHex()}
  />
...
<ColorKeyboard
    options={{ scaleX: 1, scaleY: 1, range: ["C2", "C6"] }}
    colorizer={note => rainbow(note).toHex()}
  />
...
<State initial={{ offset: 60, width: 1.4, rotation: 180, saturation: 80 }}>
  ...
  <label><input
      ...
      />rotation {state.rotation}</label><br/>
  <label><input
        min="0"
        max="100"
        type="range"
  ...
  <label><input
      ...
      />saturation {state.saturation}</label><br/>
      <p>angry rainbow</p>
<ColorKeyboard
    options={{ scaleX: 1, scaleY: 1, range: ["C2", "C6"] }}
    colorizer={note => noteColor(note, state.rotation).desaturate(100-state.saturation).lighten(noteLightness(note,state.offset,1/state.width)).toHex()}
  />
  <p>sinebow</p>
<ColorKeyboard
    options={{ scaleX: 1, scaleY: 1, range: ["C2", "C6"] }}
    colorizer={note => rainbow(note, state.rotation).desaturate(100-state.saturation).lighten(noteLightness(note,state.offset,1/state.width)).toHex()}
  />
  <p>less angry rainbow</p>
<ColorKeyboard
    options={{ scaleX: 1, scaleY: 1, range: ["C2", "C6"] }}
    colorizer={note => sinebow(note, state.rotation).desaturate(100-state.saturation).lighten(noteLightness(note,state.offset,1/state.width)).toHex()}
  />
  </>
  )}
</State>

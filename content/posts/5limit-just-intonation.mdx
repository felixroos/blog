...
<BlockMath>{`5^{n} * 2^{-2n} = (\\frac{5}{4})^{n}`}</BlockMath>
...
<BlockMath>{`\\frac{1}{1} \\approx \\frac{125}{64} * 2^{-1} \\approx \\frac{15625}{4096} * 2^{-2}`}</BlockMath>
...
<BlockMath>{`\\frac{125}{64} : 2 = \\frac{125}{128}`}</BlockMath>
...
<BlockMath>{"R_{5} = 2^{a} * 3^{b} * 5^{c}"}</BlockMath>
...
>
  {({ state, setState }) => {
    ...
    const l5 = limit5(
      [state.fifths, state.rotateFifths],
      [state.thirds, state.rotateThirds]
    )
    const flat = flatten(l5).filter(
      ({ fifth, third }) =>
        !state.disabled.find(([x, y]) => x === fifth && y === third)
    )
    const maxThirds = 3
    const maxFifths = 11
    const base = 440
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            ...
            <Slider
              ...
              onChange={(e, v) =>
                v !== state.rotateFifths && setState({ rotateFifths: v })
              }
            />
          </Grid>
        </Grid>
        <Grid container spacing={2}>
          ...
          <Grid item xs={6}>
            ...
            <Slider
              ...
              onChange={(e, v) =>
                v !== state.rotateThirds && setState({ rotateThirds: v })
              }
            />
          </Grid>
        </Grid>
            ...
            {Array.from({ length: state.fifths + 1 }, (_, f) => (
              <tr key={f}>
                {Array.from({ length: state.thirds + 1 }, (_, t) => {
                  ...
                  const [top, bottom] = new Fraction(value)
                    .toFraction()
                    .split("/")
                  const isDisabled = !!state.disabled.find(
                    ([x, y]) => fifth === x && third === y
                  )
                  const colProps = {
                    key: `${f}-${t}`,
                  ...
                  if (state.activeUnit === "frequency") {
                    return (
                      <td {...colProps}>
                        {Math.round(value * base * 10) / 10}Hz
                      </td>
                    )
                  }
                })}
...
- From each field, the field below and above are pure fifths apart, while the fields to the left and right are pure major thirds apart.
- The bounds for both grid dimensions can be adjusted with sliders on the left
- The rotation for each dimension can be adjusted with the sliders. It controls the offset of the exponents in that direction.
- You can switch between different units to change the representation type of each ratio
- Each ratio can be disabled/enabled by clicking. This will hide the ratio in the circle and gray it out.

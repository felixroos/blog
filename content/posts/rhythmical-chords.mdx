...
import {
  ...
  voicingsInRange,
  voicingReducer
} from '../components/rhythmical/dictionary/voicings.ts';
...
- ([Tone.js Playback](https://felixroos.github.io/blog/rhythmical-playback/))
- ([SVG Piano Roll Component](https://felixroos.github.io/blog/svg-piano-roll/))
- ([Sampled Instruments](https://felixroos.github.io/blog/sampler-instruments/))
...
<Player
  ...
  events={renderRhythmObject({
    ...
    .reduce(tieReducer, [])
    .reduce(voicingReducer(lefthand, ['G4', 'G6']), [])}
/>
...
These are typical [rootless lefthand voicings](https://www.youtube.com/watch?v=5uoD6LfaDoE).
...
Now we reach to lowest possible note (Bb2) on the second Cm7 (the long one). This is kind of low, but still more or less acceptable. What's less acceptable, is the last chord:
...
Now this seems to work perfectly, the only jump remains at the repetition, which could by regenerating the voicing each round, which I will implement in the future.

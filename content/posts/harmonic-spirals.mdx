...
Spirals are a good vizualization for pitch, as they convey octave relationships through rotational alignment.
A Spiral can also be seen as a 2D version of a ([pitch](https://www.researchgate.net/profile/Diana_Deutsch/publication/265168331/figure/fig15/AS:668769319919629@1536458439596/The-helical-model-of-pitch-Musical-pitch-is-shown-as-varying-along-both-a-linear.png))
helix, where the 3rd dimension of the helix is mapped to the radius of the spiral.
This gets obvious if we use a logarithmic spiral, which looks like the view inside a helix with a central vanishing point.
  ...
  {({ start, time, isRunning }) => (
    ...
    >
      {({ state, setState }) => {
        ...
        linearity = easeCubicIn(1 - time.progress);
        if (isRunning && linearity) {
          hideLines = true;
        ...
        if (!isRunning) {
          hideLines = hideLines === undefined ? false : state.hideLines;
        }
        return (
          <>
            <div
            ...
            >
              <HarmonicSpiral
                ...
                stroke="gray"
                {...state}
                {...{ zoom, spin, compression, hideLines }}
...
- make equivalence factor adjustable (other than 2), which could be used to animate from a number line (equivalence factor approaching infinity) to the spiral (2)

...
The above implementation is a general purpose path finding algorithm.
    ...
    return mapNestedArray(
      ...
      (chord) => {
        if (typeof chord === 'string') {
          const [backgroundColor] = colors.find(([color, min, max]) => i >= min && i <= max);
          ++i;
          return <div style={{ backgroundColor }}>{chord}</div>;
        }
        return chord;
      }
    );
...
<AstarChordScaleFinder chords={hhtm} scales={hhtmScales} />
<br />
...
<Card elevation={3}>
  <CardContent style={{ maxHeight: 500, overflow: 'auto' }}>
    <Graph
      options={{ fit: true, height: 600, width: 600 }}
      json={{
        graph: {
          directed: true,

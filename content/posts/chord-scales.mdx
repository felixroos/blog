...
To be visualize the algorithm for development, we will use [graphviz](https://graphviz.org/).
...
```js
export default function PathGraph(chords, allowedScales = scaleModes('major'), halfDifference = false) {
  const scales = chords

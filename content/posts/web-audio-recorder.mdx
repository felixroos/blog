...
```js
function useMediaRecorder({ onStart, onStop, onData, constraints }) {
  ...
  async function start(timeslices) {
    const stream = await getStream(); // request stream using our custom hook
    audioChunks.current = []; // clear audio chunks
    const _recorder = new MediaRecorder(stream);
    setRecorder(_recorder);
    onStart && onStart(_recorder); // callback
    _recorder.start(timeslices); // start recording with timeslices
    // called every timeslices (ms)
    _recorder.addEventListener('dataavailable', (event) => {
      audioChunks.current.push(event.data); // push new chunk
      onData && onData(event.data, audioChunks.current); // callback
    });
    _recorder.addEventListener('stop', () => onStop && onStop(audioChunks.current)); // callback
  }
  function stop() {
    if (recorder) {
      recorder.stop();

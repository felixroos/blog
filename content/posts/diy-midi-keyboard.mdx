...
Looking from the side, we can see plastic fins that push down a buttons when keys are pressed:

![switch](./img/maudio49/switch.jpeg)

To get a better view, I removed the pcb from the keybed and looked at the switches:

![switch inside](./img/maudio49/switch-inside.jpeg)

Inside each button, there are two circles with conductive ink above a snakelike pattern of conductive ink on the pcb.
If the buttons are pressed, the circles touch the snakes, acting as switches.

### Why are there two switches per key?

There are two switches per key to be able to measure the velocity of the keystroke.
As one circle is slightly more elevated, it will make a connection a little before the less elevated one.
...
I took pictures of the other (more interesting) side of the pcb, sewed them together and then redrew the copper lines above the photo:
